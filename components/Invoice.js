...
const Invoice = ({ order }) => {
  ...
  const total = parseFloat(
    orderData.items.reduce((sum, i) => sum + (Number(i?.total) > 0 ? Number(i?.total) : (Number(i.price) * Number(i.qty))), 0).toFixed(2)
  );
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: "right" }}>
            <p style={{ fontSize: "1.2rem", color: "#F9C60A", }}>
        ...
        >
          ...
          <tbody>
            ...
            {frozenItems?.length > 0 && (
              <>
                ...
                {frozenItems?.map((item, idx) => (
                  <tr
                    key={`frozen-${idx}`}
                    style={{ borderBottom: "1px solid #e5e7eb", breakInside: 'avoid', }}
            ...
            <tr style={{ borderBottom: "1px solid #e5e7eb", breakInside: 'avoid', }}>
              <td
                style={{
                  padding: "0.5rem",
                  border: "1px solid #e5e7eb",
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              ...
              <span style={{ width: "100px", textAlign: "right" }}>
                ${parseFloat(Number(order.total) + (Number(order?.totalTax) || 0)).toFixed(2)}
              </span>
            </div>
          </div>
        </div>
